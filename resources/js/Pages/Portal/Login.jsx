...
export default function Login() {
    ...
    return (
        <div className="grid min-h-screen place-items-center p-4">
            ...
            <form onSubmit={(e) => { e.preventDefault(); post('/portal/login'); }}
                  className="w-full max-w-sm rounded-lg border border-line bg-surface p-6">
                <h1 className="text-2xl font-semibold">Portal klien</h1>
                ...
                <label className="mb-4 flex items-center gap-2 text-xs text-muted">
                    <input type="checkbox" checked={data.remember} onChange={(e) => setData('remember', e.target.checked)} /> Ingat saya
                </label>
